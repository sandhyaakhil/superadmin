...
<script src="{{ asset('adm') }}/dist/js/sweetalert.min.js"></script>
                  ...
                  <tbody class="list">
                    ...
                    <tr id="row{{$setting->id}}">
                     ...
                     <td>
                        ...
                        <input class="payment_option" type="checkbox" id="payment{{ $setting->id }}" name="{{ $setting->name }}" value="1" {{ $setting->is_enabled == '1'? 'checked': '' }}>
                        <form class="{{$setting->is_enabled == '0' ? 'hidden' : '' }}" id="config_section{{$setting->id}}" name="payment_settings">
                          <span class="error{{ $setting->id }} hidden">Please enter value(s)</span>
                              <input type="hidden"  id="payment_gateway_id{{ $setting->id }}" value = "{{ $setting->id }}" name="paypal_gateway_id"/>
                              <input type="hidden"  name="gateway" value ="{{$setting->keyword}}"/>

                                <div class="form-group">
                                    <input type="text" placeholder ="Client ID" class="full-width form-control" id="{{$setting->keyword}}_client" name="{{$setting->keyword}}_client" value="{{ old('name',isset($setting->gatewaySetting->client_id)?$setting->gatewaySetting->client_id: '') }}" required/>
                                </div>
                                <div class="form-group">
                                    <input type="text" placeholder ="Secret" class="full-width form-control" id="{{$setting->keyword}}_secret" name="{{$setting->keyword}}_secret"  value="{{ isset($setting->gatewaySetting->secret)? $setting->gatewaySetting->secret:'' }}"/>
                                </div>
  ...
  $(document).ready(function(){
    ...
    {
        var error = false;
        var url = '{{ route("admin.payment_settings.save_configurations", ":id") }}';
